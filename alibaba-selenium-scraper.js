...
// Helper function to extract data from a product container
...
const scrapePage = async (driver, productDataList, index) => {
    ...
    for (const productContainer of filteredContainers) {
        ...
        const productData = {
            "Product Name": await extractData(productContainer, ".search-card-e-title a span"),
            ...
            "Image Link": await extractData(productContainer, "a.search-card-e-slider__link img.search-card-e-slider__img", "src"),
            "Product Link": await extractData(productContainer, ".search-card-e-title a", "href")
        };
...
const main = async () => {
    ...
    try {
        // Get user input
        const userInput = await question("Enter the product you want to search for: ");
        const maxPriceInput = await question("Enter the maximum price you want to filter by: ");
        ...
        await driver.get(`https://www.alibaba.com/trade/search?fsb=y&mergeResult=true&ta=y&tab=all&searchText=${searchQuery}&pricet=${maxPriceInput}`);
